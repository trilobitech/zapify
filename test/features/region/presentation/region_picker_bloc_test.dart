...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockGetRegionsByTermUseCase(term: term))
        .thenAnswer((_) async => [brazil]);

    when(() => mockGetRegionsByTermUseCase())
  ...
  test(
    'state should be RegionPickerState() when load',
    () {
      expectLater(
  ...
  group('fetchRegionsByTerm', () {
    test(
      'should call GetRegionsByTermUseCase with term "$term"',
      () async {
  ...
  test(
    ...
    () {
      expectLater(
        regionPickerBloc.stream,
        emits(RegionPickerAction.close()),
      );
