...
class AddFrameCommand extends ICommand {
  ...
  Future<void> execute() async {
    ...
    final skinName = argResults!.option('skin')!;
    final skinPath = path.join(sdkHome, 'skins', skinName);
    final layoutFile = File(path.join(skinPath, 'layout'));
    final layout = await layoutFile.readAsString();
    final parsedLayout = parseSkinLayout(layout);
    final bgImage = File(
      path.join(skinPath, parsedLayout['parts.portrait.background.image']),
    );
    final fgImage = File(
      path.join(skinPath, parsedLayout['parts.portrait.foreground.mask']),
    );
  ...
  Future<void> _addSkin(
    ...
    final outputPath = screenshotPath.replaceFirst('/android/', '/$skinName/');
